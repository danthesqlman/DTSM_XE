USE master;
GO
--This IS needed IF setting TO SAVE TO a FILE, currently we talked about setting TO APPLICATION Log
DECLARE @sql nvarchar(max)

IF NOT EXISTS (SELECT 1 FROM sys.server_audits  WHERE name = '<Audit_Name, nvarchar(128), Audit_Name>')
	BEGIN
		DECLARE @guid uniqueidentifier = NEWID()
		SET @sql = 
		N'CREATE SERVER AUDIT [<Audit_Name, nvarchar(128), Audit_Name>]
		TO APPLICATION_LOG WITH
		(    
		    QUEUE_DELAY = 1000
			,ON_FAILURE = CONTINUE
			,<Audit_Name, nvarchar(128), Audit_Name>_GUID = ''' + CONVERT(nvarchar(100),@guid) + '''
		)WHERE ([database_name]<>''tempdb'')'
		EXEC sp_executesql @sql
		ALTER SERVER AUDIT <Audit_Name, nvarchar(128), Audit_Name>  WITH (STATE = ON)
	END

IF NOT EXISTS (SELECT 1 FROM sys.server_audit_specifications WHERE name = '<Audit_Name, nvarchar(128), Audit_Name>')
	BEGIN
		SET @sql = N'
		CREATE SERVER AUDIT SPECIFICATION [<Audit_Name, nvarchar(128), Audit_Name>]
		FOR SERVER <Audit_Name, nvarchar(128), Audit_Name> [<Audit_Name, nvarchar(128), Audit_Name>]
...
IF NOT EXISTS (SELECT 1 FROM sys.dm_xe_sessions WHERE name = '<Audit_Name, nvarchar(128), Audit_Name>_Data')
	BEGIN
		CREATE EVENT SESSION [<Audit_Name, nvarchar(128), Audit_Name>_Data] ON SERVER 
		...
		ADD TARGET package0.event_file(SET filename=N'<Audit_Name, nvarchar(128), Audit_Name>_Data',max_rollover_files=(0))
	...
	ALTER EVENT SESSION <Audit_Name, nvarchar(128), Audit_Name>_Data
		ON SERVER
		STATE=START;
	END
...
USE master;
GO
DECLARE @emailrecipients nvarchar(500) = 'email@email.com' -- email address or distrobution you want to assign
DECLARE @smtpprofilename nvarchar(100) = 'Agent Mail' -- Name of the dbmail profile to be sent from. 
...
SET @SQL = 
N'CREATE TRIGGER srvr_<Audit_Name, nvarchar(128), Audit_Name>SysAdmin
	ON ALL SERVER
	...
	IF EVENTDATA().value(''(/EVENT_INSTANCE/RoleName)[1]'', ''nvarchar(max)'') = ''sysadmin''
		BEGIN
			...
			BEGIN CATCH
				--error catching
			END CATCH
		--Insert into <Audit_Name, nvarchar(128), Audit_Name>Table if you wish.
